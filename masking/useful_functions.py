...
def mask_it(mappe,path,field,nside_low=16,nside_high=2048,mode=0):
    ...
    field=[x,y, ..] where all elements are between 0 - 7 corresponding to
        ...
        with 99% sky coverage meaning that 99% of the sky is visible
        eg field=[0,1] means i will apply the 20% and 40% masks
    nside_low: nside of the final map
    nside_high: nside of the input mask
    mode:
    ...
    channels=mappe.shape[-1] #really is n_channels*pol
    npix=mappe.shape[1]
    if mode==0:
        ...
        for k in range(len(field)):
            ...
            for i in range(n_train):
                for j in range(channels):
                    map_masked = hp.ma(mappe[i,...,j])
                    map_masked.mask = maschera_neg[:]
                    new_map=map_masked.filled(fill_value)
        ...
        maschere_neg=np.logical_not(maschere_d)
        for i in range(n_train):
            k=np.random.randint(0,len(field),1)
            for j in range(channels):
                map_masked = hp.ma(mappe[i,...,j])
                map_masked.mask = maschere_neg[k][:]
                new_map=map_masked.filled(fill_value)
                mappe_out[i,...,j]=new_map[:]
    return mappe_out