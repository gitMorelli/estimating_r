...
base_dir='/home/amorelli/r_estimate/B_maps_white_noise/'+name+'/sigma/'
base_dir_tau='/home/amorelli/r_estimate/B_maps_white_noise/'+name+'/'
...
norm=f_train["norm"]
model_tau = keras.models.load_model(home_dir_tau+test_model_folder) 
...
if norm:
    ...
    count,red=nuf.check_y(y_train_sigma)
...
#print(model_tau.summary())
model_sigma=models.Sequential()
model.add(model_tau)
model_tau.trainable=False
for layer in model_tau.layers[-2:]:
...
set_trainable=False
for layer in model_tau.layers: #this set trainable all layers from layer "" to the end
    if layer.name=="":
        set_trainable=True
    if set_trainable:
        layer.trainable=True
...
model.save(base_dir+'test_model_sigma')