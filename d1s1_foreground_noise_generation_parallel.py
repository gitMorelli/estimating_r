...
home_dir="/marconi_work/INF23_indark/amorell2/d1s1_noise_generation/"
comm  = MPI.COMM_WORLD
nproc = comm.Get_size()
rank  = comm.Get_rank()

unit=-3 #i rescale the maps in mk to prevent the simple_comp_sep overflow
...
T_maps=uf.generate_maps(data=data, r=r,n_train=n_maps,nside=16, map_per_cl=maps_per_cl, noise_maps=noise_T, beam_w=beam_w, kind_of_map="TT", raw=1, n_channels=2, beam_yes=1 , verbose=0)[0]
...
for i in range(n_maps):
    for k in range(n_channels):
        compsep=basic_comp_sep(components, instrument, freq_maps[k,i],nside=nside).s[0]
        for p in range(pol):
            result[i,:,k*pol+p]=compsep[p+1]*10**(-unit)-QU_maps[i,:,k*pol+p] #compsep is in mK and mappe_QU in uK. I give the output in uK
